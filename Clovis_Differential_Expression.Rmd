...
untar( "data.tgz" )
midgut_tsv <- read.table("https://c-moor.github.io/data/midgut.tsv", header=TRUE)
...
In the previous lab, we looked at a single HT-seq file, which provides gene expression data for one sample. Let's consider an example from Marianes and Spalding et al. Recall that they compared many RNA-seq samples from different regions of the midgut in *Drosophilia melanogaster* (Figure 2A, from [Marianes and Spradling 2013](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3755342/))

![Figure 2A](https://c-moor.github.io/images/elife-00886-fig2A.jpg){ width=100% }
**Figure 1A** Diagram illustrating the different RNA-seq libraries constructed by Marianes and Spalding 2013 from the Drosophila midgut.

From an HT seq file, we can learn that the *lab* gene is highly expressed in the Cu region of the *Drosophlia* midgut. But, does that mean the *lab* gene is important for cells of the Cu region? Maybe, maybe not. It could be that *lab* is expressed at that level across the whole midgut or possible even the whole fly. To really answer questions about gene expression, we need to compare more than one sample with each other. Which samples are compared depends on the scientific question being asked. In terms of the *lab* gene, Marianes and Spalding analyzed its expression in each region of the gut and determined that it is localized to the Cu region (Figure 1D, from [Marianes and Spradling 2013](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3755342/)). This is a clue that *lab* is important in the Cu region.

![Figure 2D](https://c-moor.github.io/images/elife-00886-fig2D.jpg){ width=100% }
**Figure 1D** Analysis of seven genes and their gene expression across each region of the gut.

In today's lab we will learn a hand-full of methods for looking at gene expression across the *Drosophila* midgut. As you work through the lab, keep your group's scientific question in mind. You will not need all of the methods to answer your question. Also think about which samples you will need to compare to answer your question.
...
1. First, use the `results()` command to get the relevant data.  The `results()` command needs to know which samples you want to compare.  Make sure to store this output in a variable.
  + In the example below, we have names this variable `temp` (short for temporary) because it's not in the right format yet.
...
### Getting results for a single gene
...
+ You can adjust how many results appear on the plot using `showCategory`.  It's a good idea to find out how many gene categories you have (using the `dim` command on the results from `clusterProfiler` and then set `dotplot` to show all of them)
...
dotplot(a1_vs_p1_clusters, showCategory=29, title="a1 vs p1")
```
...
```{r clusterProfiler-practice, exercise=TRUE}
```
